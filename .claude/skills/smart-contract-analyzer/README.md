...
1. [math-analyzer.md](.claude/agents/math-analyzer.md) — Solidity does not support float type which leads to a lot of issues with division and rounding and this subagents aims to spot them.
2. [signature-verification-analyzer.md](.claude/agents/signature-verification-analyzer.md) — Covering different attack vectors with signatures on-chain verification such as signature replay, DoS, etc.
3. [oracle-analyzer.md](.claude/agents/oracle-analyzer.md) — Covering Chainlink's and Pyth's potential issues during integration and fetching of price feed data.
